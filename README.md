...
![Tests](https://img.shields.io/badge/tests-421%20passing-3B8C6E)
...
- **KPI tiles and page headers are SVG drawn by DAX measures**
  (`dataCategory: ImageUrl`). Each tile shows its value, a composition rail and
  a one-line context, all from existing measures, with no custom visual to
  install.
- **A metric switch**, a button slicer on a disconnected `Chart Metric` table,
  flips the wave, landing-zone, pipeline, criticality and concentration charts
  between artifacts, planned days and actual days. Their titles are measures,
  so each chart always names the metric it is showing.
- **A pop-up filter panel** holds the domain, source-type and criticality
  slicers and is opened and closed by bookmarks. The bookmarks capture
  visibility only, so the panel never resets a filter, and the page header and
  the Filters button (`Filters · 2`) keep the filter state on screen while the
  panel is closed.
- **Status pills and progress rails inside tables**: a verdict pill on every
  validation run, % migrated and effort status on every wave.
- **One colour map across the report**: GO is green and NO-GO red, and each
  status, complexity and criticality keeps the same colour in every chart and
  tile.

The report passes Microsoft's `powerbi-report-author validate` with no errors
or warnings, and every page was rendered and clicked through in Power BI
Desktop. [`tests/test_report_interactions.py`](tests/test_report_interactions.py)
pins the ways these patterns fail silently: an unescaped `%` turns every SVG
fill black, a bookmark that also captures data resets the filters, and a
button pointing at a deleted bookmark does nothing.
